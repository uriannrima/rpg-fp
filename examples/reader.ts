 ...
 * And keep maing Either
 */
const getSkillAbilityScore2 = flow(
  getSkillWithReader,
  R.map(flow(E.map(getKeyScore), E.chain(getAbilityScore2)))
);
...
const as4 = getSkillAbilityScore2("No Skill")(eInstance); // Right({ name: Intelligence, value:12 })
const as5 = getSkillAbilityScore2(SkillType.Bluff)(eInstance); // Right({ name: Intelligence, value:12 })

console.log({ as1, as2, as3, as4, as5 });
